...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const [previewDoc, setPreviewDoc] = useState<{ url: string; title: string; fileName: string; viewOnly?: boolean } | null>(null);
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 space-y-6">
          ...
          {activeTab === 'dispatch' && (
            <div className="space-y-6">
              ...
              {/* Proof of Delivery - view only (partners) */}
              {isAgent && (
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <h3 className="font-semibold text-xs text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  {podCopyUrl && (podCopyUrl.startsWith('data:') || podCopyUrl.startsWith('http')) ? (
                    <div className="flex items-center justify-between gap-3 text-xs">
                      <div className="min-w-0">
                        <p className="text-[11px] text-slate-400">Signed delivery slip</p>
                        <p className="font-semibold text-slate-800 truncate" title={podCopyFileName || 'Proof of Delivery'}>
                          {podCopyFileName || 'Proof of Delivery'}
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() =>
                          setPreviewDoc({
                            ...
                            fileName: podCopyFileName || 'Proof of Delivery',
                            viewOnly: true
                          })
                        }
                        className="shrink-0 px-4 py-2 rounded-lg bg-gradient-to-br from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-xs font-semibold flex items-center gap-1.5 shadow-sm shadow-emerald-700/20 transition-all"
                      >
                    ...
                    <p className="text-xs text-slate-400">The POD is still loading - please close and reopen this order in a moment.</p>
                  ) : (
                    <p className="text-xs text-slate-400">The proof of delivery has not been uploaded for this order yet.</p>
                  )}
                </div>
              )}
